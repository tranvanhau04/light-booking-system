...
// === 1. TÁCH IMPORT CONTEXT ===
import { useAuth } from '../context/AuthContext'; // <-- Lấy user từ đây
import { useBooking } from '../context/BookingContext'; // <-- Lấy booking data từ đây

const ProfileScreen = ({ navigation }: any) => {
  // === 2. LẤY DỮ LIỆU TỪ 2 CONTEXT ===
  // Lấy user, hàm logout, và trạng thái loading từ AuthContext
  const { user, authLogout, isLoading } = useAuth(); 
  // Lấy hàm dọn dẹp booking từ BookingContext
  const { clearBookingData } = useBooking(); 

  // === 3. CẤU TRÚC MENU (Giữ nguyên) ===
  const menuSections = [
    {
      title: 'Account',
      items: [
...
        style: 'destructive',
        onPress: () => {
          if (authLogout) authLogout(); // 1. Đăng xuất (xóa user khỏi AuthContext)
          if (clearBookingData) clearBookingData(); // 2. Xóa dữ liệu booking
          
...
  // === 6. HIỂN THỊ LOADING (CẬP NHẬT) ===
  // Hiển thị loading nếu AuthContext đang tải hoặc chưa có user
  if (isLoading || !user) {
    return (
...
  // === 7. GIAO DIỆN CHÍNH (Sử dụng 'user' từ AuthContext) ===
  return (
...
            </View>
            {/* Sửa 'user.name' thành 'user.fullName' để khớp với AuthContext */}
...
    // Bỏ paddingTop để dùng SafeAreaView hoặc header
  },
  loadingContainer: {
    flex: 1, // Sửa để loading container chiếm toàn màn hình
